...
**The digit rule.** Divide the unit cube into a $3\times3\times3$ grid and index each subcell by $(x,y,z)$ with each coordinate in $\{0,1,2\}$. Let $\text{mid}=(x{=}1,\,y{=}1,\,z{=}1)$ record which coordinates sit at the centre. Every grid sponge is then one predicate on that digit vector — a hand-listed cell set would obscure the pattern, but as a predicate the whole family is a single rule applied recursively, each surviving cell subdividing into its own $3\times3\times3$ block:
...
**Corner sponges (Sierpinski tetrahedron / octahedron).** These are point-contact fractals: at each step every existing copy is replaced by half-scale copies placed at the *vertices* of the seed solid, so where a grid sponge removes material a corner fractal instead clusters shrunken selves at the extremities. Starting from a centre set $\{0\}$ and halving the scale each step,
