...
**The solids.** With cylinders of radius $r$ along the coordinate axes, the bicylinder is the set of points inside both perpendicular cylinders, $\{x^2+y^2\le r^2\}\cap\{x^2+z^2\le r^2\}$, and the tricylinder intersects a third, adding $\{y^2+z^2\le r^2\}$. Each inequality is "inside this cylinder", and taking them together keeps only the points common to all of them.
