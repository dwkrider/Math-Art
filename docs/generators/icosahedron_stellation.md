...
**2. Enumerate the cells.** Twenty planes cut space into many regions, and each region is labelled by its **sign vector** $s\in\lbrace+1,-1\rbrace^{20}$ — for every plane, which side of it the region lies on. The core cell is "inside all twenty" ($s=(-1,\dots,-1)$); stepping across one plane flips one sign. The engine walks outward from the core by breadth-first search, flipping one sign at a time, and keeps only the **bounded** regions — a large bounding cube catches the unbounded ones (any region touching the cube runs to infinity and is discarded). This search is the expensive step, and it is what makes the output a genuine derivation rather than a transcription: for the icosahedron it finds exactly **473 bounded cells**, the classical Du Val/Maeder count.
