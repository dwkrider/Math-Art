...
# SEAM and ANTI are solved for (Route I, intrinsic); TRUNCATE is built
# exactly by slicing a polyhedron (Route II, extrinsic, see
# `conical.py`); the rest are closed-form relatives (`analytic.py`)
MODES = ('SEAM', 'ANTI', 'TRUNCATE') + analytic.ANALYTIC_KINDS
CONICAL_KINDS = ('TRUNCATE',)
...
def _build_conical(mode, scale, seed, edges, rounds, depth):
