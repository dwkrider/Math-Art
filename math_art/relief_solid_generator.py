...
if _IN_BLENDER:
    ...
    _PRESETS = {
        'PLANET': dict(
            base='SPHERE', res=5, field='FRACTAL', method='FBM', hurst=0.75,
            octaves=9, curve='NONE', depth=0.26, seed=3),
        'ASTEROID': dict(
            base='SPHERE', res=5, field='FRACTAL', method='WEIERSTRASS',
            dim=2.4, octaves=8, curve='NONE', depth=0.45, seed=7),
        'HARMONIC_BALL': dict(
            # The sphere's own eigenmode: a Chladni figure for a ball.
            base='SPHERE', res=5, field='HARMONIC', sph_l=6, sph_m=3,
        ...
        'SHAGREEN': dict(
            ...
            base='SPHERE', res=5, field='CELLULAR', points_n=220,
            cell_mode='CRACK', cell_sharp=0.7, curve='NONE', depth=0.2,
            seed=11),
        'WOVEN_TORUS': dict(
            base='TORUS', res=192, field='GABOR', gabor_freq=9.0,
            points_n=260, spread=0.4, curve='NONE', depth=0.16, seed=5),
        'CORAL_TORUS': dict(
            base='TORUS', res=192, field='CELLULAR', points_n=260,
        ...
        'CARVED_COLUMN': dict(
            ...
            base='CYLINDER', res=192, field='GABOR', gabor_freq=7.0,
    ...
    class MESH_OT_relief_solid_add(bpy.types.Operator):
        ...
        base: EnumProperty(
            ...
            default='SPHERE')
        res: IntProperty(
            name="Detail", default=5, min=1, max=400,
            description="Icosphere subdivisions for the sphere (each step "
                        "quadruples the faces); grid resolution otherwise")
        ...
        def _params(self):
            return dict(
                base=self.base, res=self.res, ring=self.ring, tube=self.tube,
        ...
        def draw(self, context):
            ...
            col.prop(self, 'base')
            col.prop(self, 'res')
            if self.base == 'SPHERE' and self.res > 6:
                col.label(text="Each step quadruples the faces",
                          icon='INFO')
            elif self.base == 'TORUS':
                col.prop(self, 'ring')
                col.prop(self, 'tube')
...
def _selftest():
    ...
    for name in ('PLANET', 'ASTEROID', 'HARMONIC_BALL', 'SHAGREEN',
        ...
        preset = {
            'PLANET': dict(base='SPHERE', res=3, field='FRACTAL'),
            'ASTEROID': dict(base='SPHERE', res=3, field='FRACTAL',
                             method='WEIERSTRASS'),
            'HARMONIC_BALL': dict(base='SPHERE', res=3, field='HARMONIC',
                                  sph_l=6, sph_m=3),
            'SHAGREEN': dict(base='SPHERE', res=3, field='CELLULAR',
                             points_n=80),
            'WOVEN_TORUS': dict(base='TORUS', res=64, field='GABOR',
                                points_n=80),
            'CORAL_TORUS': dict(base='TORUS', res=64, field='CELLULAR',
                                points_n=80),
            'CARVED_COLUMN': dict(base='CYLINDER', res=64, field='GABOR',
                                  points_n=80),
        }[name]
    ...
    for base in _B:
        for fld in ('FRACTAL', 'CELLULAR', 'GABOR', 'HARMONIC', 'LATTICE'):
            v, f, _ = build_solid(base=base, res=3 if base == 'SPHERE' else 48,
                                  field=fld, points_n=60, seed=1)
