...
if _IN_BLENDER:
    ...
    # Kowalewski's scheme for the rhombic triacontahedron takes five
    # colours, every block using three of them, so that the ten 3-subsets
    # each appear once as an acute block and once as a flat one.  Applied
    # to any dissection, a block's colour comes from its own zone triple,
    # so blocks generated by the same three zones match.  The palette
    # itself is shared with the other zonohedral generators -- the whole
    # point of the colouring is that a zone family looks the same wherever
    # it is drawn.
    try:
    ...
    _PALETTE = _fc.PALETTE
    _material = _fc.material
